...
// Some devices need to send vendor specific requests before configuration.
// Therefore, this constraint must be removed.
//    if ((usbHostState & STATE_MASK) != STATE_RUNNING)
//    {
...
void USBHostShutdown( void )
{
    ...
    // overcurrent situation.
		...
		#ifdef __PIC32MX__
	        /*****************
	         * Some of the masks of the PIC32 reset the USB peripheral differently
	         * Because of this the SFRs will also be manually cleared so that the
	         * state of the peripheral is the same from mask to mask.  For masks that
	         * fully reset the part, this will not have any side effects
	         ************************/
...
BOOL _USB_FindServiceEndpoint( BYTE transferType )
{
    ...
    {
        if ( usbDeviceInfo.pEndpoint0->countNAKsInFrame < 20 )
        { 
            pCurrentEndpoint = usbDeviceInfo.pEndpoint0;
            return TRUE;
        }
...
//#else
//    #error Cannot define USB interrupt vector.
//#endif

void USBHostISR ( void )
{
        ...
        (!(U1IEbits.UERRIE && U1IRbits.UERRIF) || (pCurrentEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS)))
    {
        ...
        if ((pCurrentEndpoint->bEndpointAddress & 0x0F) == (U1TOK & 0x0F))
        {
            ...
            if (pBDT->STAT.PID == PID_ACK)
            {
                ...
                pCurrentEndpoint->countNAKsInFrame = 0;
            ...
            else if (pBDT->STAT.PID == PID_NAK)
            {
                ...
                pCurrentEndpoint->countNAKsInFrame ++;
    ...
    if (U1IEbits.SOFIE && U1IRbits.SOFIF)
    {
        ...
        #ifndef ALLOW_MULTIPLE_NAKS_PER_FRAME
            usbDeviceInfo.pEndpoint0->status.bfLastTransferNAKd = 0;
        #endif
        usbDeviceInfo.pEndpoint0->countNAKsInFrame = 0;
